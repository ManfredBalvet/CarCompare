...
def __crawl_models(manufacturer_url: str) -> list[str]:
    ...
    if download_url(manufacturer_url) == "":
        models = ""
    else:
        url_start = manufacturer_url.split("https://www.guideautoweb.com")[1]
        pattern_start = "href=\"" + __escape_regex_pattern(url_start)
        pattern_end = "\""
        pattern = pattern_start + "([^'\"]+)" + pattern_end
...
def __crawl_years(model_url: str) -> list[str]:
    ...
    if download_url(model_url) == "":
        ...
        pattern_start = "<option value=\"" + __escape_regex_pattern(url_start)
        pattern_end = "\""
        pattern = pattern_start + "([^'\"]+)" + pattern_end
...
def __get_specs_url(year_url: str) -> str:
    ...
    if download_url(year_url) == "":
        specs_url = ""
    else:
        url_start = year_url.split("https://www.guideautoweb.com")[1]
        pattern_start = __escape_regex_pattern("href=\"" + url_start + "specifications/")
        pattern_end = "\""
        pattern = pattern_start + "([^'\"]+)" + pattern_end
...
def __escape_regex_pattern(pattern: str) -> str:
    characters_to_escape = ["\\", "*", "+", "?", ".", "^", "[", "]", "$", "&", "|"]
    for character_to_escape in characters_to_escape:
        pattern = pattern.replace(character_to_escape, "\\" + character_to_escape)
